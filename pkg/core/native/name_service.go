...
func checkName(rt recordtypes.Type, name string) {
	var valid bool
	switch rt {
	case recordtypes.A:
...
func (n *NameService) resolveInternal(ic *interop.Context, name string, t recordtypes.Type, redirect int) (string, bool) {
	if redirect < 0 {
	...
	data, ok := records[recordtypes.CNAME]
	if !ok {
		return "", false
	}
...
func (n *NameService) getRecordsInternal(d dao.DAO, name string) map[recordtypes.Type]string {
	domain := toDomain(name)
	key := makeRecordKey(domain, name, 0)
	key = key[:len(key)-1]
	res := make(map[recordtypes.Type]string)
	d.Seek(n.ID, key, func(k, v []byte) {
		rt := recordtypes.Type(k[len(k)-1])
		res[rt] = string(v)
	})
	return res
}

func makeRecordKey(domain, name string, rt recordtypes.Type) []byte {
...
func toRecordType(item stackitem.Item) recordtypes.Type {
	...
	switch rt := recordtypes.Type(val); rt {
	case recordtypes.A, recordtypes.CNAME, recordtypes.TXT, recordtypes.AAAA:
		return rt
	default:
		panic("invalid record type")
	}
